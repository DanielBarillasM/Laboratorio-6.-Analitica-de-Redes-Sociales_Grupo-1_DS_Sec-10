...
La modularidad fue 0.395. La comunidad 1 reúne 126 autores y 161 comentarios, con vocabulario sobre pueblo, dinero y diputados, y sentimiento negativo (score promedio $-0.72$ sobre 59 comentarios con léxico). La comunidad 2 contiene 61 autores y 83 comentarios relacionados con presidencia, Guatemala y gobierno, con sentimiento positivo ($+0.89$). La comunidad 3 contiene 55 autores y 60 comentarios, destacando USAC, universidad y corrupción, con sentimiento levemente positivo ($+0.62$). Estas comunidades reflejan agrupación por contenidos compartidos; no son grupos sociales confirmados. El detalle de sentimiento por comunidad está en \texttt{resumen\_comunidades.csv} y se explica en la Sección~\ref{sec:sentimiento}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{sentimiento_distribucion.png}
\caption{Distribución de sentimiento en los comentarios analizados.}
\end{figure}
...
La participación observada está altamente concentrada (Sección~4.1) y la estructura de redes confirma ese patrón: la red autor--video es un conjunto de camarillas por video, unidas por muy pocos autores puente (9 de 332), y el sentimiento negativo se concentra precisamente en el video que actúa como articulador estructural, \emph{Qué rico come tu diputado}. Esto es consistente con una dinámica típica de YouTube en contenido político: pocos videos concentran participación, visibilidad y polarización, mientras el resto del corpus queda fragmentado y mayormente aislado. El contenido institucional/gubernamental, cuando recibe comentarios, tiende a ser más positivo.

La red revela \emph{quién} conecta qué contenidos; el análisis de sentimiento revela \emph{qué tono} domina esas conexiones. Ambas lecturas están acotadas por una cobertura parcial de comentarios, un muestreo por consultas y una identificación de sentimiento con cobertura del 36.9\% del corpus. Las conclusiones se limitan a esta muestra observada y no a la totalidad de la conversación pública en YouTube sobre estos temas ni a la población general de Guatemala.

\section{Reproducibilidad}

El flujo se ejecuta en tres pasos: \texttt{python scripts/run\_advance.py} (ejercicios 1--4 y parte de 5--7), \texttt{python scripts/run\_network\_final.py} (proyecciones, topología, comunidades y ejercicio 8 de centralidad/puentes) y \texttt{python scripts/run\_sentiment.py} (ejercicio 9 de sentimiento). El notebook se reconstruye con \texttt{python scripts/build\_notebook.py}. Las tablas y figuras se generan automáticamente en \texttt{outputs/}. El cargador acepta CSV real y libros Excel aunque la extensión sea incorrecta. La suite automatizada (\texttt{pytest tests -q}) valida lectura, reparación de IDs, conteos, limpieza y significado de pesos. Código, dependencias e instrucciones están disponibles en \repo.
