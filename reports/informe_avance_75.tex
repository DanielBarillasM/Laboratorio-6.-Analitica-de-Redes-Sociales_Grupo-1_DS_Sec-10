...
El archivo \texttt{youtube\_videos\_12808423.csv} tiene extensión CSV, pero su firma binaria corresponde a un libro Excel. El cargador detecta el formato por contenido y acepta tanto este caso como el CSV verdadero que se incorpore posteriormente. Tres identificadores que inician con guion fueron almacenados como fórmulas de Excel y aparecían vacíos al usar el lector habitual; se reconstruyeron de manera auditable desde el parámetro \texttt{v} de \texttt{video\_url}.
...
Se evaluaron dimensiones, tipos, valores faltantes, duplicados, constantes, atípicos y consistencia de IDs, nombres y handles. No hay duplicados completos. Tras reparar los tres \texttt{video\_id}, las llaves son completas y únicas. \texttt{viewer\_rating} está vacío en los 406 comentarios y \texttt{is\_pinned} es constante; el primero se excluye y el segundo se conserva solo para auditoría. Las fechas relativas no se transforman en fechas exactas.
